...
GKMODULE(top_bar_t)
{
	MODULE_INST(PC_de_Nana, gk_button_cb);
	...
	MODULE_INST(quitter, gk_menu_entry);
	MODULE_INST(texte, gk_menu_entry);
	MODULE_INST(calcul, gk_menu_entry);
	//MODULE_INST(entry21, gk_menu_entry);
	//MODULE_INST(entry31, gk_menu_entry);	
	GKCTOR(top_bar_t) 
	{
		PC_de_Nana.menu_button = 1; // no frame
		//PC_de_Nana.caption.set_font_div(1);
		//menu0.add_entry(go_term);
		//menu0.add_entry(go_curve);
		//menu0.add_entry(go_fuck_yourself);
		menu1.add_entry(texte);
		menu1.add_entry(calcul);
		menu1.add_entry(quitter);
		//menu1.add_entry(entry31);
	}
	
	void layout()
	{
		split<HOR>(160, menu1, PC_de_Nana);
...
gstring roblox = "Roblox";
gstring internet = "Internet";
gstring devoirs = "Devoirs";
GKMODULE(top_if_t)
{
	GKCTOR(top_if_t) 
	{
		...
		kb1.set_invisible();
		//popup.set_text("\n\nJoyeux anniversaire \nNana !\n\n13 septembre 2018");
		popup.msg.set_font_div(1);
		popup.msg.bg_color = 0xff3f3f3f;
		popup.msg.auto_scale = 0;
		btn1.caption.set_buffer(&roblox);
		btn2.caption.set_buffer(&internet);
		btn3.caption.set_buffer(&devoirs);
	...
	MODULE_INST(btn1, gk_button_cb);
	MODULE_INST(btn2, gk_button_cb);
	MODULE_INST(btn3, gk_button_cb);
	...
	//LINK_REMOTE(box_signal, top_bar, PC_de_Nana);
	...
	void layout()
	{
		...
		popup.shrink<VERT>(128);
		popup.shrink<HOR>(128);
		...
		titi.split<VERT>(128, btn1, dummy);
		btn1.split<VERT>(128, btn1, btn3);
		btn1.split<VERT>(128, btn1, btn2);
		btn3.split<VERT>(128, btn3, dummy);
		//btn3.set_toggle(1);	
		//btn1.set_toggle(1);	

		SET_CALLBACK(top_bar.quitter, top_if_t, quit);
		...
		SET_CALLBACK(top_bar.texte, top_if_t, keyboard);
		SET_CALLBACK(top_bar.calcul, top_if_t, calc);
		SET_CALLBACK(btn1, top_if_t, msg1);
		SET_CALLBACK(btn2, top_if_t, msg2);
		SET_CALLBACK(btn3, top_if_t, msg3);
		SET_CALLBACK(top_bar.PC_de_Nana, top_if_t, msg0);
		...
		term.print_line("Bienvenue !!!\n");
		term.print_line("Sur le super PC de Nana\n");
		calc_app.term2.print_line("GRUIK CALC v0.0\n");
	}
	void msg0()
	{
				term.print_line("Joyeux anniv, Nana.\n");

		popup.set_text("\nJoyeux anniversaire Nana ! \n13 septembre 2018\n\nOk");
		popup.signal_event(2);
	}
	void msg1()
	{
				term.print_line("RAPPEL : se laver les dents.\n");
	...
	void msg2()
	{
				term.print_line("RAPPEL : faire ses devoirs.\n");
	...
	void msg3()
	{
				term.print_line("RAPPEL : refaire ses devoirs.\n");
